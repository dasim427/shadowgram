...
private enum SGExtrasEntry: ItemListNodeEntry {
    ...
    case link(Int32, Int32, String, Int)
    ...
    var section: ItemListSectionId {
        ...
        case let .tweakHeader(_, section, _), let .tweakToggle(_, section, _, _, _), let .tweakInfo(_, section, _), let .replacementRules(_, section, _), let .roundOption(_, section, _, _, _, _), let .bubbleTails(_, section, _, _), let .tweakAction(_, section, _, _), let .link(_, section, _, _):
            return section
        }
    }
    ...
    var stableId: Int32 {
        ...
        case .exportInfo:
            return 21
        case let .tweakHeader(id, _, _), let .tweakToggle(id, _, _, _, _), let .tweakInfo(id, _, _), let .replacementRules(id, _, _), let .roundOption(id, _, _, _, _, _), let .bubbleTails(id, _, _, _), let .tweakAction(id, _, _, _), let .link(id, _, _, _):
            return id
        }
    }
...
private func sgExtrasEntries(state: SGExtrasState) -> [SGExtrasEntry] {
    ...
    entries.append(.tweakHeader(720, appearance, "БЕЙДЖИ И ИКОНКИ"))
    entries.append(.link(721, appearance, "Бейджи профиля", 8))
